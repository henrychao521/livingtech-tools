...
const STEPS = [
  ...
  { title: '插入 LED', anim: 'led', desc: '把 LED 長腳（陽極）跟電阻另一端接同一行（例如 j10）。短腳（陰極）插另一行（例如 j13）。', tip: '不確定哪腳長就拿到光下看：LED 內部有兩塊金屬，較大的那塊是負極（陰極）、較小的尖頭是正極（陽極）。', warn: '插反的話 LED 不會亮但不會壞。發現不亮要先檢查方向。' },
