...
const STEPS = [
  ...
  },
  {
    ...
    warn: '鋸齒裝反（齒尖朝上）不是切不動而已：往上拉的切削力會把木板從工作台抬起、跟著鋸條上下震彈，既切不準又危險。這是初學者最常見的錯誤。',
  },
  {
    title: '調整速度',
    anim: 'speed',
    desc: '依木材種類調整：硬木（如柚木、橡木）→ 慢速；軟木（如松木）→ 中速；合板是膠合板材，依厚度判斷，薄合板可用中速；切曲線需要時可再加快。',
  ...
  },
  {
    title: '對準切割線',
    anim: 'align',
    desc: '把鋸條對準切割線的「廢料側」（一般外輪廓是線的外側；內挖時廢料在線的內側），保留線約 1mm，後續用銼刀修平。確認手部位置在「安全三角區」（雙手拇指在木板邊緣、距鋸條至少 5 公分）。',
