...
const SCENARIOS = [
  ...
  { q: '電池（12V 鉛酸）保管的正確方式？', a: '隨意疊放在工具櫃角落', b: '直立放在通風的電池架上、正負極端子加蓋、編號管理', correct: 'b', explain: '鉛酸電池橫放可能漏液，端子裸露被金屬短路會引發火災。每顆電池都要編號追蹤循環次數。' },
  { q: '機器人測試（test mode）時，誰可以接近機器人？', a: '所有隊員都可以圍觀', b: '只有 driver 與 1 名安全觀察員，其他人保持 2m 以上距離', correct: 'b', explain: 'FRC 機器人馬達瞬間扭力極大（如 NEO 馬達 11Nm），失控可造成嚴重夾擊傷害。測試時必須建立「安全圈」。' },
  { q: '焊接電子線材時，工作站應該？', a: '在通風櫃裡或靠窗處 + 排煙裝置', b: '在電池附近方便取電', correct: 'a', explain: '焊接煙含助焊劑揮發物（松香、有機溶劑），長期吸入有害。電池會釋放氫氣，靠近焊接火花極危險。' },
  { q: 'CAD 設計的零件交給製造組前必須？', a: '直接送去 CNC 加工', b: '先 peer review、確認 tolerance、檢查刀具是否能加工', correct: 'b', explain: 'Tolerance 不對會讓零件組裝不起來；刀具半徑限制 = 內角圓角必須大於刀徑。Citrus Circuits 規定 2 人簽字才能進製造。' },
  { q: '比賽期間機器人異常冒煙，立刻？', a: '繼續比賽看會不會自己好', b: '駕駛員按下急停 → 通知裁判 → 等技術組到場處理', correct: 'b', explain: '冒煙通常是馬達控制器（如 Talon SRX）短路。繼續通電可能引發火災，FRC 場上有 CO2 滅火器但更重要的是先斷電。' },
  ...
  { q: 'PIT 區整理工具的原則？', a: '常用工具放最方便拿的位置', b: 'Shadow board（影子板）+ 工具回歸定位，每場比賽前盤點', correct: 'b', explain: '頂尖隊伍如 Team 254 用「影子板」每個工具有指定位置。比賽中工具掉進機器人會卡住馬達。比賽前必須清點。' },
  { q: 'CIM、NEO、Falcon 等 FRC 馬達操作注意？', a: '通電後可用手測試轉動方向', b: '通電前先確保軸上沒有手指/物體；通電後保持 30cm 距離', correct: 'b', explain: 'Falcon 500 空載扭力 4.7 Nm，瞬間啟動可夾斷手指。「斷電才動機構」是黃金原則。' },
