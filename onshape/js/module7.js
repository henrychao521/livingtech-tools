...
const QUIZ = [
  { q: '匯入動物圖片後發現比例不對（太小），下一步該？', opts: ['用「尺寸」工具設定目標尺寸（如 120mm）', '重新匯入', '直接 Extrude 看看', '重畫'], correct: 0, hint: '用「尺寸」工具就能精準設定圖片大小，不必重新匯入。' },
  ...
  { q: '前後兩隻腳要分開列印（雷射切割兩片），最好用什麼方式建立兩個草圖平面？', opts: ['偏移平面（Offset Plane）建立兩個不同 z 位置的平面', '畫在同一平面再分離', '用 Loft 連接', '不可能做到'], correct: 0, hint: '偏移平面是 Onshape 的標準做法。書中設定 ±15mm 偏移，兩腳就有前後距離。' },
  { q: '要把拼圖送到雷射切割機，應該匯出什麼格式？', opts: ['STL（3D 列印用）', 'PDF（文件用）', 'DXF（2D 雷射切割用）', 'OBJ（動畫用）'], correct: 2, hint: 'DXF 是 2D 向量圖檔，雷射切割機標準輸入格式。STL 是 3D 列印用。' },
];
